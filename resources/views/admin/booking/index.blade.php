                    ...
                    <tbody>
                        @foreach ($bookings as $booking)
                            <tr>
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">{{ $booking->lapangan->nama }}</p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">
                                        {{ \Carbon\Carbon::parse($booking->booking_date)->translatedFormat('d M Y') }}<br>
                                        {{ \Carbon\Carbon::parse($booking->timeSlot->jam_mulai)->format('H:i:s') }} 
                                        {{ \Carbon\Carbon::parse($booking->jam_selesai)->format('H:i:s') }}
                                    </p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <span class="relative inline-block px-3 py-1 font-semibold leading-tight">
                                        ...
                                        "></span>
                                        <span
                                            class="relative capitalize">{{ str_replace('_', ' ', $booking->status) }}</span>
                                    </span>
                                </td>
